...
/**
 * Marketing landing page — a NEW route that does not touch the existing root
 * ("/", the candidate login). Intended to be served on its own marketing
 * domain, which maps to this path via a Vercel/Next rewrite (see the PR
 * report). It lives in this codebase purely so it inherits the Nocturne
 * tokens, the Logo component and the bundled fonts — the header/hero styling
 * is its own `.lp-*` namespace in globals.css, kept apart from the app chrome.
 *
 * Sign-in targets: candidates go to the app root ("/"), employers to
 * "/employer". Both are relative by default (marketing page + app are the
 * same Next deployment, so a relative link resolves within it). If the app
 * is ever served from a different origin than the marketing domain, set
 * NEXT_PUBLIC_APP_ORIGIN and both links absolutize to it — reported so the
 * hosts can be confirmed.
 */
const APP_ORIGIN = process.env.NEXT_PUBLIC_APP_ORIGIN ?? '';
const CANDIDATE_HREF = `${APP_ORIGIN}/`;
