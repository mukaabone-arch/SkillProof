...
- **Automatic, fail-hard at container startup:** `apps/api/docker-entrypoint.sh` runs `prisma migrate deploy` before starting the app in every container, exiting non-zero (never starting the app) if migrations fail — so ECS's circuit breaker rolls back instead of ever serving traffic against a schema the running code doesn't match. Manual migration application (temporarily opening the RDS security group to a developer's IP) should no longer be necessary; if you find yourself doing it, that's a sign this mechanism broke.
- **Canary deploys mean migrations must be additive and backwards-compatible.** Deploys are canary-based (5% traffic, 3-minute bake) — the previous task version keeps running and serving real traffic for the whole bake window, and it never re-runs migrations itself. A migration that drops or renames a column the *previous* release's code still reads/writes will break that old task mid-deploy, before the bake even completes. Split destructive changes (drop/rename) across two deploys: one that stops the old code from reading/writing the column (ship the new code first, migration additive-only), a later one that actually removes it once nothing running references it anymore.
- **Advisory-lock timeout is a known, accepted failure mode, not a mystery to re-diagnose.** Prisma Migrate wraps every `migrate deploy` in a Postgres advisory lock so concurrent applies (e.g. two container starts landing close together) serialize instead of racing — safe by construction. But the lock wait has a fixed, non-configurable 10-second timeout; if one instance holds it for longer than that (a slow migration), a second instance's own `migrate deploy` will fail with a `pg_advisory_lock` timeout error and that container will exit non-zero too — not because anything is actually wrong with the schema, just lock contention. If you see this in CloudWatch during a deploy, it's this, not a corrupted migration: let the circuit breaker roll back and retry rather than treating it as a schema emergency.
